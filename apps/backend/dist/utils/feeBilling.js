 ...
 */
export async function generateStudentFeeSchedule(studentId, schoolId, academicYear, adminSupabase) {
    ...
    const { data: cycles, error: cyclesError } = await adminSupabase
        ...
        .eq('is_active', true);
    if (cyclesError)
        throw new Error(`Failed to get fee cycles: ${cyclesError.message}`);
    ...
    if (!cycles || cycles.length === 0) {
        ...
        if (createError)
            throw new Error(`Failed to create default cycle: ${createError.message}`);
        cycles.push({
            student_id: studentId,
            school_id: schoolId,
            fee_cycle: 'monthly',
 ...
 */
export async function getPendingPeriods(studentId, adminSupabase) {
    ...
    if (error)
        throw new Error(`Failed to get pending periods: ${error.message}`);
    return data || [];
}
/**
 * Get overdue periods for a student
 */
export async function getOverduePeriods(studentId, adminSupabase) {
    const { data, error } = await adminSupabase
        .from('fee_bill_periods')
        .select(`
      *,
    ...
    if (error)
        throw new Error(`Failed to get overdue periods: ${error.message}`);
    return data || [];
}
/**
 * Get total dues for a student
 */
export async function getStudentTotalDues(studentId, adminSupabase) {
    const { data, error } = await adminSupabase
        .from('fee_bill_periods')
        .select('status, expected_amount, paid_amount, balance_amount')
        .eq('student_id', studentId);
    if (error)
        throw new Error(`Failed to get student dues: ${error.message}`);
    const periods = data || [];
    const result = {
        total_periods: periods.length,
        paid_periods: periods.filter((p) => p.status === 'paid').length,
        pending_periods: periods.filter((p) => ['pending', 'billed', 'partially-paid', 'overdue'].includes(p.status)).length,
        total_expected: periods.reduce((sum, p) => sum + parseFloat(p.expected_amount || 0), 0),
        total_paid: periods.reduce((sum, p) => sum + parseFloat(p.paid_amount || 0), 0),
        total_balance: periods.reduce((sum, p) => sum + parseFloat(p.balance_amount || 0), 0),
        overdue_amount: periods
            .filter((p) => p.status === 'overdue')
            .reduce((sum, p) => sum + parseFloat(p.balance_amount || 0), 0)
    };
    return result;
}
/**
 ...
 */
export function calculateDueDate(periodStart, periodEnd, dueDay) {
 ...
 */
export async function markOverduePeriods(adminSupabase) {
