...
async function fetchAllStudentOverridesAsOf(studentId, asOfDate, adminSupabase) {
...
function latestOverrideByFeeCategory(overrides) {
    const sorted = [...overrides].sort((a, b) => String(b.effective_from).localeCompare(String(a.effective_from)));
    const m = new Map();
...
function pickOverrideForCategory(overrideMap, feeCategoryId) {
    const k = feeCategoryId == null ? '' : String(feeCategoryId);
    return overrideMap.get(k) ?? null;
}
function applyOverrideToAmount(baseAmount, override) {
    if (!override)
        return baseAmount;
    if (override.is_full_free)
        return 0;
    if (override.custom_fee_amount != null)
        return parseFloat(override.custom_fee_amount);
    if (override.discount_amount)
        return Math.max(0, baseAmount - parseFloat(override.discount_amount));
    return baseAmount;
}
/**
 ...
 */
export async function loadAssignedFeeStructureAsOf(studentId, schoolId, adminSupabase, asOfDate, opts) {
    const dateStr = toLocalYmd(asOfDate);
    let student;
    if (opts?.student) {
    ...
    const result = { class_fees: [], custom_fees: [] };
    ...
    if (student.class_group_id) {
      ...
      `)
        ...
        const sorted = [...(versionRows || [])].sort((a, b) => String(b.effective_from_date).localeCompare(String(a.effective_from_date)));
        const byKey = new Map();
        for (const row of sorted) {
        ...
        for (const row of byKey.values()) {
            const catId = row.fee_category_id;
            ...
            const name = (row.fee_categories && (Array.isArray(row.fee_categories) ? row.fee_categories[0]?.name : row.fee_categories?.name)) ||
                'Class Fee';
    ...
    if (studentTransport?.route_id && student.class_group_id) {
        ...
        const routeName = typeof routeNameRaw === 'string'
            ? routeNameRaw
            : routeNameRaw?.route_name;
        if (routeName) {
            ...
            const tSorted = [...(tvRows || [])].sort((a, b) => String(b.effective_from_date).localeCompare(String(a.effective_from_date)));
    ...
    if (customFeeCategories?.length) {
      ...
      `)
        ...
        for (const [, rows] of groups) {
            ...
            const catName = (catJoin && (Array.isArray(catJoin) ? catJoin[0]?.name : catJoin?.name)) || 'Custom Fee';
...
export async function generateMonthlyFeeComponents(studentId, schoolId, year, month, feeStructure, adminSupabase) {
    ...
    const classLines = feeStructure.class_fees.length > 0
        ? feeStructure.class_fees
        : feeStructure.class_fee
            ? [feeStructure.class_fee]
            : [];
    for (const cf of classLines) {
        if (!cf || !shouldBillThisMonth(cf.fee_cycle, cf.start_date))
            continue;
        const amount = cf.fee_cycle === 'monthly'
            ? cf.amount
...
export async function monthlyFeeComponentRowExists(adminSupabase, params) {
    let q = adminSupabase
        .from('monthly_fee_components')
        .select('id')
...
export async function generateMonthlyFeeComponentsForStudent(studentId, schoolId, adminSupabase, targetYear, targetMonth) {
    ...
    for (let year = admissionYear; year <= endYear; year++) {
        ...
        for (let month = startMonth; month <= monthLimit; month++) {
            ...
            const hasAny = feeStructure.class_fees.length > 0 ||
