...
router.post('/structure', requireRoles(['principal']), async (req, res) => {
    ...
    try {
        ...
        let effectiveFromDate = value.effective_from_date;
        ...
        // Invalidate cache after salary structure update
        await invalidateCache(`school:${user.schoolId}:salary:unpaid:*`);
...
router.post('/generate', requireRoles(['principal', 'clerk']), async (req, res) => {
    ...
    try {
      ...
      `)
        ...
        // Invalidate cache after salary generation
        await invalidateCache(`school:${user.schoolId}:salary:unpaid:*`);
...
router.post('/payments', requireRoles(['clerk']), async (req, res) => {
    ...
    try {
        ...
        const expectedSalary = salaryStructure.base_salary +
            salaryStructure.hra +
      ...
      `)
        ...
        if (excessAmount > 0) {
            try {
                ...
                const { data: creditResult, error: creditError } = await adminSupabase.rpc('apply_salary_credits_to_future_months', {
        ...
        // Invalidate cache after salary payment
        await invalidateCache(`school:${user.schoolId}:salary:unpaid:*`);
        return res.json({
            payment,
...
router.get('/unpaid', requireRoles(['principal', 'clerk']), async (req, res) => {
    ...
    try {
        ...
        const cacheKey = `school:${user.schoolId}:salary:unpaid:${time_scope || 'last_12_months'}:${teacher_id || 'all'}`;
...
async function fetchUnpaidSalaries(supabase, user, queryParams) {
    try {
        ...
        const teachersList = Array.from(teacherMonthsMap.entries()).map(([teacherId, months]) => {
            const summary = (unpaidTeachersSummary || []).find((s) => s.teacher_id === teacherId);
            ...
            return {
                ...
                teacher_email: months[0]?.teacher_email || '',
                ...
                unpaid_months: sortedMonths.map((m) => ({
                    month: m.month,
                    year: m.year,
...
router.get('/credits/:teacherId', requireRoles(['principal', 'clerk', 'teacher']), async (req, res) => {
    ...
    try {
        ...
        // Get credit balance
        const { data: creditBalance, error: creditError } = await adminSupabase.rpc('get_teacher_credit_balance', {
