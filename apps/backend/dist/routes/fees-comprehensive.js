        ...
        for (const student of students) {
            ...
            if (classFees) {
                for (const classFee of classFees) {
                    const shouldInclude = (classFee.fee_cycle === 'one-time' && !classFee.due_date) ||
            ...
            if (transportAssignment) {
                const { data: transportFee } = await adminSupabase
                    ...
                    .single();
                if (transportFee) {
                    const shouldInclude = (transportFee.fee_cycle === 'monthly') ||
            ...
            if (optionalFees) {
                for (const optionalFee of optionalFees) {
                    const shouldInclude = (optionalFee.fee_cycle === 'one-time') ||
            ...
            if (customFees) {
                for (const customFee of customFees) {
                    const shouldInclude = (customFee.fee_cycle === 'per-bill') ||
