...
const router = Router();
const SCHOOL_SELECT = 'id, name, join_code, registration_number, address, contact_email, contact_phone, logo_url, created_at';
...
router.get('/info', requireRoles(['principal', 'clerk']), async (req, res) => {
    ...
    try {
        ...
        const { data: school, error: schoolError } = await supabase
            ...
            .single();
        if (schoolError) {
            logger.warn({ userId, schoolId: profile.school_id, code: schoolError.code }, '[school/info] School fetch failed');
