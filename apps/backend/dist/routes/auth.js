...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        ...
        if (accessToken) {
            ...
            logger.info({ userId: user.id }, '[reset-password] Session revocation skipped because no access token is available');
        }
...
function otpRateRedisKey(profileId) {
    return `otp:rate:${profileId}`;
}
function otpIpRedisKey(ip) {
    return `otp:ip:${ip}`;
}
function generateOTP() {
    return crypto.randomInt(100000, 1000000).toString();
}
function clientOtpIp(req) {
...
async function enforceOtpRequestRateLimit(profileId) {
...
async function enforceOtpIpRateLimit(req) {
...
router.post('/forgot-password-request-email', async (req, res) => {
    ...
    try {
        ...
        const otpKey = otpRedisKey(profile.id);
        const existingOtpEmail = await redis.get(otpKey);
        if (existingOtpEmail) {
            return res.json({ message: 'If an account exists with this email, an OTP has been sent.' });
        }
        const { otp } = await storePasswordResetOtp({
...
router.post('/forgot-password-verify', async (req, res) => {
    ...
    try {
        ...
        logger.info({ profileId }, '[forgot-password-verify] Session revocation skipped because no access token is available');
