 ...
 */
async function fetchStudentsSummary(schoolId) {
    if (!schoolId) {
 ...
 */
async function fetchStudentsData(user, queryParams) {
    try {
      ...
      `)
        ...
        const requestedLimit = parseInt(queryParams.limit) || 50;
        ...
        if (class_group_id && user.role !== 'teacher') {
            countQuery.eq('class_group_id', class_group_id);
        }
        if (section_id) {
            countQuery.eq('section_id', section_id);
        }
      ...
      `)
        ...
        if (student.class_group_id) {
            ...
            if (classFees && classFees.length > 0) {
                ...
                const classFeeOverride = feeOverrides?.find((fo) => fo.fee_category_id === classFees[0].fee_category_id);
                if (classFeeOverride) {
        ...
        const transportCategory = feeOverrides?.find((fo) => fo.fee_categories?.fee_type === 'transport');
        ...
        const otherFees = (allFeeCategories || []).map((cat) => ({
        ...
        if (studentClassId) {
            ...
            if (customFeesData) {
                ...
                const customFeeCategoryIds = (customFeesData || []).map((cf) => cf.fee_category_id).filter(Boolean);
                let customFeeOverrides = [];
                ...
                customFees = (customFeesData || []).map((cf) => {
                    // Find override by matching fee_category_id
                    const override = customFeeOverrides.find((o) => o.fee_category_id === cf.fee_category_id);
                    return {
                        custom_fee_id: cf.id,
    ...
    try {
        ...
        if ((classChanged || fee_config) && finalClassId) {
            ...
            `)
                ...
                const [routeResult, transportCategoryResult] = transportData;
                if (classFees && classFees.length > 0) {
                    let selectedClassFee = null;
                    if (feeConfigToApply.class_fee_id) {
                        selectedClassFee = classFees.find((cf) => cf.id === feeConfigToApply.class_fee_id);
                    }
                    else {
                ...
                if (transportEnabled && routeResult.data && !routeResult.error) {
                    ...
                    if (feeConfigToApply.transport_fee_discount > 0 && transportCategory) {
                        transportInserts.push(adminSupabase
                            .from('student_fee_overrides')
                            .insert({
                            student_id: studentId,
                            school_id: user.schoolId,
                ...
                if (feeConfigToApply.other_fees && feeConfigToApply.other_fees.length > 0) {
                    const otherFeeOverrides = feeConfigToApply.other_fees
                        .filter((otherFee) => otherFee.enabled && otherFee.discount > 0)
                        .map((otherFee) => ({
                        student_id: studentId,
                        school_id: user.schoolId,
                ...
                if (feeConfigToApply.custom_fees && feeConfigToApply.custom_fees.length > 0) {
                    ...
                    if (customFeeIds.length > 0) {
                        ...
                        if (!customFeeDefsError && customFeeDefs) {
                            ...
                            const feeCategoryIds = [...new Set(customFeeDefs.map((cf) => cf.fee_category_id).filter(Boolean))];
                            ...
                            const customFeeOverrides = [];
                            for (const customFee of feeConfigToApply.custom_fees) {
                                const customFeeDef = customFeeDefs.find((cf) => cf.id === customFee.custom_fee_id);
