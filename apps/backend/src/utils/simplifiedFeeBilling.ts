 ...
 */
async function getStudentFeeOverride(
  studentId: string,
 ...
 */
async function calculateCategoryFee(
  studentId: string,
  feeCategoryId: string,
  defaultFee: number,
  periodDate: Date,
 ...
 */
export async function calculateStudentFeesSimplified(
  studentId: string,
  classGroupId: string,
  schoolId: string,
  ...
  finalAmount: number;
  breakdown: FeeCalculationResult[];
}> {
  const dateStr = periodDate.toISOString().split('T')[0];
  const breakdown: FeeCalculationResult[] = [];
  ...
  for (const feeDefault of classFeeDefaults) {
    ...
    const calculation = await calculateCategoryFee(
      studentId,
      feeCategoryId,
      defaultFee,
      periodDate,
  ...
  if (transportAssignment) {
    ...
    if (transportFeeDefault) {
      ...
      if (transportCategory) {
        ...
        const transportCalculation = await calculateCategoryFee(
          studentId,
