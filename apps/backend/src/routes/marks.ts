...
router.post('/bulk', requireRoles(['teacher', 'principal', 'clerk']), async (req, res) => {
  ...
  try {
    ...
    const invalidRecords = value.marks.filter((m: any) => m.school_id !== user.schoolId);
    ...
    if (user.role === 'teacher') {
      const subjectIds = [...new Set(value.marks.map((m: any) => m.subject_id))] as string[];
      ...
      const assignedSubjectIds = assignments.map((a: any) => a.subject_id);
      const unauthorizedSubjects = subjectIds.filter((id: string) => !assignedSubjectIds.includes(id));
    ...
    const studentIds = [...new Set(value.marks.map((m: any) => m.student_id))];
    ...
    const marksData = value.marks.map((m: any) => ({
...
router.get('/results', requireRoles(['clerk', 'principal']), async (req, res) => {
  ...
  try {
    ...
    if (class_group_id || section_id) {
      ...
      if (class_group_id) {
        studentQuery = studentQuery.eq('class_group_id', class_group_id as string);
      }
      if (section_id) {
        studentQuery = studentQuery.eq('section_id', section_id as string);
      }
    ...
    if (exam_id) {
      query = query.eq('exam_id', exam_id as string);
    }
    if (subject_id) {
    ...
    let filteredData = (data || []).filter((mark: any) => {
    ...
    filteredData.sort((a: any, b: any) => {
      ...
      if (examA !== examB) return examB.localeCompare(examA);
...
router.get('/teacher', requireRoles(['teacher']), async (req, res) => {
  ...
  try {
    ...
    const classIds = [...new Set(assignments.map((a: any) => a.class_group_id))];
    ...
    if (class_group_id) {
      query = query.eq('students.class_group_id', class_group_id as string);
    }
    if (subject_id) {
      query = query.eq('subject_id', subject_id as string);
    }
    if (section_id) {
      query = query.eq('students.section_id', section_id as string);
    }
