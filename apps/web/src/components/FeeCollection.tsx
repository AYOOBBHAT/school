...
export default function FeeCollection() {
  ...
  const loadClasses = async () => {
    ...
    try {
      ...
      devLog('[FeeCollection] Loaded classes', { count: classesList.length });
  ...
  const loadStudents = async () => {
    ...
    try {
      ...
      if (requestId !== latestRequestRef.current) {
        devLog('[FeeCollection] Ignoring stale request result');
        setLoadingStudents(false);
        return;
      }
      
      devLog('[FeeCollection] Received students payload', {
      ...
      if (data.classes && Array.isArray(data.classes)) {
        data.classes.forEach((cls: any) => {
          if (cls.students && Array.isArray(cls.students)) {
            ...
            devWarn(`[FeeCollection] Class ${cls.name} has no students array or it's not an array:`, cls);
          }
        });
      } else {
        devWarn('[FeeCollection] No classes array in response or it\'s not an array:', data.classes);
      }
      ...
      devLog(`[FeeCollection] Extracted ${studentsList.length} students from response`);
      ...
      if (requestId === latestRequestRef.current) {
        ...
        devLog(`[FeeCollection] Updated allStudents with ${studentsList.length} students (total from API: ${totalFromAPI})`);
        ...
        if (!searchQuery.trim()) {
          devLog(`[FeeCollection] No search query, setting ${studentsList.length} students immediately`);
      ...
      if (allStudents.length === 0 && students.length === 0) {
        devError('[FeeCollection] loadStudents failed');
      }
    } finally {
  ...
  const applySearchFilter = (studentList: Student[], query: string) => {
    if (!query.trim()) {
      devLog(`[FeeCollection] No search query, setting all ${studentList.length} students`);
    ...
    devLog('[FeeCollection] Search filter applied', { matched: filtered.length, total: studentList.length });
  ...
  useEffect(() => {
    ...
    if (loadingStudents) {
      devLog('[FeeCollection] Debounced effect skipped - loading students');
      return;
    }
    
    // Don't filter if allStudents is empty
    if (allStudents.length === 0) {
      devLog('[FeeCollection] Debounced effect skipped - no students available');
    ...
    devLog(`[FeeCollection] Debounced effect triggered - allStudents: ${allStudents.length}, searchQuery: "${searchQuery}"`);
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (!loadingStudents && allStudents.length > 0) {
        devLog(`[FeeCollection] Debounced effect executing - filtering ${allStudents.length} students`);
        ...
        devLog('[FeeCollection] Debounced effect cancelled - loading or no students');
      }
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                devLog('[FeeCollection] Class filter changed');
